...
namespace SynthLab
{
	...
	*/
	bool MorphWTCore::reset(CoreProcData& processInfo)
	{
		// --- parameters
	...
	\returns true if successful, false otherwise
	*/
	bool MorphWTCore::update(CoreProcData& processInfo)
	{
		// --- parameters
		WTOscParameters* parameters = static_cast<WTOscParameters*>(processInfo.moduleParameters);
		...
		/*
			...
			For the MorphWTCore this is the morphing location
		*/
	...
	\returns true if successful, false otherwise
	*/
	double MorphWTCore::renderSample(SynthClock& clock)
	{
	...
	\returns true if successful, false otherwise
	*/
	double  MorphWTCore::renderHardSyncSample(SynthClock& clock)
	{
		double output = 0.0;
	...
	\returns true if successful, false otherwise
	*/
	bool MorphWTCore::render(CoreProcData& processInfo)
	{
		// --- parameters
		WTOscParameters* parameters = static_cast<WTOscParameters*>(processInfo.moduleParameters);
	...
	\returns true if successful, false otherwise
	*/
	bool MorphWTCore::doNoteOn(CoreProcData& processInfo)
	{
		// --- parameters
		WTOscParameters* parameters = static_cast<WTOscParameters*>(processInfo.moduleParameters);
	...
	\returns true if successful, false otherwise
	*/
	bool MorphWTCore::doNoteOff(CoreProcData& processInfo)
	{
		return true;
	}
	...
	\returns true if successful, false otherwise
	*/
	void MorphWTCore::addMorphBankData(std::string name, SynthLabBankSet& slBankSet, uint32_t index)
	{
	...
	\returns true if successful, false otherwise
	*/
	void MorphWTCore::checkAddWaveBank(SynthLabBankSet& slBankSet, CoreProcData& processInfo, uint32_t waveIndex)
	{
		for (uint32_t i = 0; i < slBankSet.tablePtrsCount; i++)
		{
	...
	*/
	int32_t MorphWTCore::checkAddWavetable(SynthLabTableSet& slTableSet, StaticTableSource* tableSource, CoreProcData& processInfo)
	{
