...
namespace SynthLab
{
	...
	0. VAOCore: uses BLEP to synthesize sawtooth and square waveforms
	1. --- EMPTY ---
	2. --- EMPTY ---
	3. --- EMPTY ---
	...
	- set the parameters in the VAOscParameters structure with new values, typically from a GUI
	...
	SynthModuleWithCores(nullptr, nullptr, 64);
	...
	- VAOscParameters: in standalone mode only, these are synthesized locally on the object,
	...
	- renders stereo by default. For VA this really means dual-mono where the
	...
	*/
	class SynthModuleWithCores : public SynthModule
	{
	public:
		...
		SynthModuleWithCores(std::shared_ptr<MidiInputData> _midiInputData,
			std::shared_ptr<SynthModuleWithCoresParameters> _parameters,
			uint32_t blockSize = 32);

		/** Destructor is empty: all resources are smart pointers */
		virtual ~SynthModuleWithCores() {}/* D-TOR */
		...
		/** For standalone operation only; not used in SynthLab synth projects
			This needs to match the constructor for your particular object.
		*/
