...
namespace SynthLab
{
	...
	0. LFOCore all the classic LFO waveforms, random sample and hold, and exponentials
	1. FMLFOCore LFO using simple FM algorithms to produce interesting LFO waveforms that
	  move more quickly through complex waveshapes
	2. --- EMPTY ---
	3. --- EMPTY ---
	...
	- getParameters() function allows direct access to std::shared_ptr<LFOParameters>

	std::shared_ptr<LFOParameters> getParameters()

	- call the getParameters() function
	- set the parameters in the LFOParameters structure with new values, typically from a GUI
	...
	- LFOParameters: in standalone mode only, these are synthesized locally on the object, 
	...
	*/
	class SynthLFO : public SynthModule
	{
	public:
		/** One and only specialized constructor; pointers may be null for stanalone */
		SynthLFO(std::shared_ptr<MidiInputData> _midiInputData,
		...
		std::shared_ptr<LFOParameters> getParameters() { return parameters; }

	protected:
		/** For standalone operation only; not used in SynthLab synth projects */
		std::shared_ptr<LFOParameters> parameters = nullptr;
	};
